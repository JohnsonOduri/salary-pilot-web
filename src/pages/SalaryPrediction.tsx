...
const SalaryPrediction = () => {
  ...
  return <div className="max-w-7xl mx-auto p-6 bg-violet-100">
					...
					<CardContent className="space-y-6">
						<div className="grid md:grid-cols-2 gap-4">
							<div className="space-y-2">
								...
								<Input id="experience" type="number" placeholder="e.g., 3" value={formData.experience} onChange={e => handleInputChange('experience', e.target.value)} />
								{formData.experience && parseInt(formData.experience) < 0 && <p className="text-sm text-red-500 mt-1">
									Experience must be at least 0
								</p>}
							{formData.experience && parseInt(formData.experience) > 35 && <p className="text-sm text-red-500 mt-1">
						...
						<div className="space-y-2">
							...
							<Input id="jobTitle" placeholder="e.g., Software Engineer, Data Scientist" value={formData.jobTitle} onChange={e => handleInputChange('jobTitle', e.target.value)} />
						</div>

						<div className="space-y-2">
							<Label htmlFor="age">Age</Label>
							<Input id="age" type="number" placeholder="e.g., 28" value={formData.age} onChange={e => handleInputChange('age', e.target.value)} />
							{formData.age && parseInt(formData.age) < 18 && <p className="text-sm text-red-500 mt-1">
									Age must be 18 or older 
								</p>}
							{formData.age && parseInt(formData.age) > 55 && <p className="text-sm text-red-500 mt-1">
									Age must be 55 or younger for 
								</p>}
							{formData.age && parseInt(formData.age) - parseInt(formData.experience) < 18 && <p className="text-sm text-red-500 mt-1">
						...
						<div className="space-y-2">
							...
							<Textarea id="skills" placeholder="e.g., React, Python, Machine Learning, SQL" value={formData.skills} onChange={e => handleInputChange('skills', e.target.value)} rows={3} />
						</div>

						<Button onClick={predictSalary} disabled={isLoading} className="w-full bg-gradient-primary hover:opacity-90" size="lg">
							{isLoading ? <>
        ...
        animationDelay: '0.2s'
					...
					{prediction ? <Card className="shadow-card border-0 bg-gradient-card">
							<CardHeader>
